    ...
    if primary_element_type:
        if primary_element_type in conns:
            ...
            try:
                ...
                raise ValueError(f"Requested element type '{primary_element_type}' not found. Available: {available}")
...
def get_geometry_from_file(filepath, element_type=None, device="cpu"):
    ...
    """
    x, conn, _, _ = load_mesh(filepath, device=device, primary_element_type=element_type)
    return x, conn
...
if __name__ == "__main__":
    ...
    path = os.path.abspath(os.path.join(script_dir, "..", "HA4_src_task", "kreisklein.msh"))
    try:
