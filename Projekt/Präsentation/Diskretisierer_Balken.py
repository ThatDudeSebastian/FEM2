...
def von_mises_return(eps2, state):
    ...
    if float(Phi_tr) <= 0.0:
        ...
        return sig2, Ct2, state_new, sig_trial[2,2]
    ...
    return sig2, Ct2, state_new, sig[2,2]
...
if __name__ == "__main__":
    print("Running Hannes_main.py simulation...")
    # ============ MESH LOADING ============
    # ==========================================
    ...
    while step <= n_steps:
        ...
        while True:
            ...
            for it in range(newton_max):
                ...
                for el in range(nel):
                    ...
                    for q in range(nqp):
                        ...
                        sig, Ct, state_gp_iter[el][q], s33 = von_mises_return(eps, state_gp_old[el][q])
                        ...
                        # Correct 3D von Mises for Plane Strain (including s33)
                        s11, s22, s12 = sig[0,0].item(), sig[1,1].item(), sig[0,1].item()
                        s33_val = s33.item()
                        svm = math.sqrt(
                            s11**2 + s22**2 + s33_val**2
                            - (s11*s22 + s22*s33_val + s33_val*s11)
                            + 3.0*s12**2
                        )
                        evm = math.sqrt(
            ...
            if converged:
                ...
                sig_tr, _, _, _ = von_mises_return(eps_tr, st_tr_q0)
    ...
    for e in tqdm(range(nel), desc="Post-processing"):
        ...
        for q in range(nqp):
            ...
            sig2, Ct2, _, s33 = von_mises_return(eps, st_q)
    
            # von Mises (3D Plane Strain)
            svm = math.sqrt(
                sig2[0,0].item()**2 + sig2[1,1].item()**2 + s33.item()**2
                - (sig2[0,0].item()*sig2[1,1].item() + sig2[1,1].item()*s33.item() + s33.item()*sig2[0,0].item())
                + 3.0*sig2[0,1].item()**2
            )
    ...
    fig_setup, ax1 = plt.subplots(figsize=(10, 8))
    fig_u, ax2 = plt.subplots(figsize=(10, 8))
    fig_s, ax3 = plt.subplots(figsize=(10, 8))
    ...
    ax1.set_xlim(-limit, limit); ax1.set_ylim(-limit, limit); ax1.set_aspect('auto')
    ...
    ax2.set_xlim(-limit, limit); ax2.set_ylim(-limit, limit); ax2.set_aspect('auto')
    ...
    ax3.set_xlim(-limit, limit); ax3.set_ylim(-limit, limit); ax3.set_aspect('auto')
    ...
    plt.subplot(2, 2, 1)
    plt.plot(disp_pl, np.array(load_hist)/1e3, 'r', lw=2, label="Plastisch")
    plt.plot(disp_pl, np.array(load_target_hist)/1e3, 'k--', lw=1, label="Target")
    ...
    plt.subplot(2, 2, 2)
    plt.plot(eps_p_xx_hist, np.array(sig_yy_hist)/1e6, lw=2, color='blue')
    ...
    plt.subplot(2, 2, 3)
    plt.plot(eps_p_eq_hist, np.array(sig_eq_hist)/1e6, lw=2, color='green')
    ...
    if len(k_hist) > 0:
        ...
        # --- Visualization of Hardening (Ellipse) ---
        # User desire: Visualize the hardening (expansion/translation) using the "Plane Stress Ellipse" concept
        # even though the actual Stress Path is constrained by Plane Strain limits.
        
        # Calculate Backstress Tensor for Yield Surface Center
        # Center X = (2/3)*(1-r)*H * a
        factor = (2.0/3.0) * (1.0 - r) * H
        beta0 = factor * a0
        beta1 = factor * a1
        ...
        # Plot Ellipses (Phantom/Reference)
        plt.plot(x0/1e6, y0/1e6, 'k--', lw=1.5, alpha=0.5, label="Initial Yield (Ref)")
        plt.plot(x1/1e6, y1/1e6, 'g--', lw=1.5, alpha=0.8, label="Hardened Surface (Ref)")
        
        # Plot Center of Yield Surface (Backstress)
        b1_center = float(beta1[0,0])
        b2_center = float(beta1[1,1])
        plt.plot(b1_center/1e6, b2_center/1e6, 'gx', markersize=8, label="Center (Backstress)")
        
        # --- Plane Strain Limits (Actual Constraint) ---
        # For Plane Strain (eps33=0), yielding occurs when |sigma1 - sigma2| >= 2/sqrt(3) * sigma_eff 
        # Shifted by the backstress difference.
        
        # Effective Yield Stress (Isotropic Hardening)
        sig_eff_current = sigma_y + r * H * k1 
        
        # We use the diagonal components of the backstress tensor for the shift 
        # (Consistent with the ellipse center)
        limit_val = (2.0 / math.sqrt(3.0)) * sig_eff_current / 1e6
        shift_diff = (b2_center - b1_center) / 1e6
        
        # Determine plot bounds based on History AND Ellipse
        if len(sig_1_hist) > 0:
            x_min_hist = np.min(sig_1_hist); x_max_hist = np.max(sig_1_hist)
            y_min_hist = np.min(sig_2_hist); y_max_hist = np.max(sig_2_hist)
            
            # Combine with ellipse bounds to ensure everything is visible
            x_min_tot = min(x_min_hist, np.min(x1/1e6))
            x_max_tot = max(x_max_hist, np.max(x1/1e6))
            y_min_tot = min(y_min_hist, np.min(y1/1e6))
            y_max_tot = max(y_max_hist, np.max(y1/1e6))
            
            cx = 0.5*(x_min_tot + x_max_tot)
            cy = 0.5*(y_min_tot + y_max_tot)
            span = max(x_max_tot - x_min_tot, y_max_tot - y_min_tot)
            if span < 10.0: span = 200.0
            
            margin = span * 0.4
            x_lo, x_hi = cx - margin, cx + margin
            y_lo, y_hi = cy - margin, cy + margin
        else:
            x_lo, x_hi = -400, 400
            y_lo, y_hi = -400, 400

        # Generate points for limit lines
        x_vals = np.linspace(x_lo, x_hi, 100)
        
        # Upper Line: s2 = s1 + shift + limit
        y_upper = x_vals + shift_diff + limit_val
        # Lower Line: s2 = s1 + shift - limit
        y_lower = x_vals + shift_diff - limit_val
        
        plt.plot(x_vals, y_upper, 'r-', lw=2.0, alpha=0.6, label="Plane Strain Limit")
        plt.plot(x_vals, y_lower, 'r-', lw=2.0, alpha=0.6)
        
        # Fill the "Safe" region
        plt.fill_between(x_vals, y_lower, y_upper, color='red', alpha=0.05)

        # Plot Stress Path Trajectory
        if len(sig_1_hist) > 0:
            plt.plot(sig_1_hist, sig_2_hist, 'b-', lw=2.0, alpha=0.9, label="Stress Path")
            plt.plot(sig_1_hist[-1], sig_2_hist[-1], 'bo', markersize=8, label="Current State")
        
        # Add labels and formatting
        plt.xlabel(r"Principal Stress $\sigma_1$ [MPa]")
        plt.ylabel(r"Principal Stress $\sigma_2$ [MPa]")
        plt.title(fr"Yield Surface (r={r:.2f})" + "\n" + r"Ellipse: Hardening State | Lines: Plane Strain Limits")
