...
if nen == 8:
    ...
    for e in range(nel): ax1.plot(x[conn[e][idx], 0]*1000, x[conn[e][idx], 1]*1000, color='black', lw=0.4, alpha=0.15)
else:
    for e in range(nel): ax1.plot(x[torch.cat([conn[e], conn[e][:1]]), 0]*1000, x[torch.cat([conn[e], conn[e][:1]]), 1]*1000, color='black', lw=0.4, alpha=0.15)

for n in fixed_nodes: ax1.scatter(x[n, 0]*1000, x[n, 1]*1000, color='green', marker='o', s=15, alpha=0.5, zorder=5)
for bc in neum_bcs:
    if abs(bc[2]) < 1e-6 or bc[1] == 0: continue
    n = int(bc[0]); xn, yn = x_np[n]*1000; vec = -x_np[n]/np.linalg.norm(x_np[n])
    ax1.arrow(xn - 0.1*limit*1000*vec[0], yn - 0.1*limit*1000*vec[1], 0.08*limit*1000*vec[0], 0.08*limit*1000*vec[1], head_width=0.015*limit*1000, head_length=0.02*limit*1000, fc='red', ec='red', zorder=6)
ax1.set_aspect('auto'); ax1.grid(True, alpha=0.3)
...
verts_mm = [x_def_mm[conn[e][idx if element_type=="quad8" else [0,1,2,3,0]]] for e in range(nel)]
...
ax2.set_title("Verschiebungsbetrag", fontweight='bold'); ax2.set_aspect('auto'); ax2.autoscale_view()
...
ax3.set_title("Von Mises Spannung", fontweight='bold'); ax3.set_aspect('auto'); ax3.autoscale_view()
...
if interactive_hover:
    ...
    def hover(event):
        for ax, ann, pc, key, unit, f in [(ax2, ann2, pc_u, "u_norm", "mm", fig_u), (ax3, ann3, pc_s, "svm", "MPa", fig_s)]:
            if event.inaxes == ax:
                ...
                if cont:
                    ...
                    ann.set_visible(True); f.canvas.draw_idle(); return
