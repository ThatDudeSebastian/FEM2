...
fixed_nodes = extract_nodes_from_sets(
    pt_sets, cell_sets, mesh_cells, ["Fixed", "Support", "Hub", "Innen"]
)
if not fixed_nodes:
...
loaded_nodes = extract_nodes_from_sets(
    pt_sets, cell_sets, mesh_cells, ["Loaded", "Last", "Tread", "Aussen"]
)
if not loaded_nodes:
    r_dist = torch.sqrt(x[:, 0] ** 2 + x[:, 1] ** 2)
...
for e in range(nel):
    for i1, i2, im in q8_edges:
        n_idx = conn[e]
        n1, n2, nm = int(n_idx[i1]), int(n_idx[i2]), int(n_idx[im])
...
for n1, n2, nm in boundary_edges:
    [b_nodes.add(n) for n in [n1, n2, nm]]
b_nodes = np.array(list(b_nodes), dtype=int)
ymin = x_np[b_nodes, 1].min()
i_contact = b_nodes[np.argmin(x_np[b_nodes, 1])]
x_contact = x_np[i_contact]
n_in = -x_contact / np.linalg.norm(x_contact)
t_hat = np.array([-n_in[1], n_in[0]])

contact_edges = []
for n1, n2, nm in boundary_edges:
    if loaded_group_set and not (
        n1 in loaded_group_set or n2 in loaded_group_set or nm in loaded_group_set
    ):
        continue
    s_loc = float(np.dot((x_np[nm] - x_contact), t_hat))
    if abs(s_loc) <= 1.2 * a_hz:
...
if len(contact_edges) > 0:
    ...
    def N_line(xi):
        return np.array([0.5 * xi * (xi - 1), 0.5 * xi * (xi + 1), 1 - xi * xi])

    def dN_line(xi):
        return np.array([xi - 0.5, xi + 0.5, -2 * xi])
...
if nen == 8:
    ...
    for e in range(nel):
        ax1.plot(
            x[torch.cat([conn[e], conn[e][:1]]), 0] * 1000,
            x[torch.cat([conn[e], conn[e][:1]]), 1] * 1000,
            color="black",
            lw=0.4,
            alpha=0.15,
        )
...
for bc in neum_bcs:
    n = int(bc[0])
    if n not in node_forces_mag:
        node_forces_mag[n] = 0.0
    node_forces_mag[n] += bc[2]**2

for n in node_forces_mag:
    node_forces_mag[n] = math.sqrt(node_forces_mag[n])

max_force = max(node_forces_mag.values()) if len(node_forces_mag) > 0 else 1.0
...
for n, f_mag in node_forces_mag.items():
    ...
    draw_scale = max(scale, 0.1)  # keep a minimum head size
    gap = 0.02 * limit * 1000
    ...
    ax1.arrow(
        ...
        arrow_length * vec[0],
        arrow_length * vec[1],
        head_width=0.015 * limit * 1000 * draw_scale,
        head_length=0.02 * limit * 1000 * draw_scale,
        fc="red",
        ec="red",
        zorder=6,
    )
...
def get_shape_data(xi, nen):
    ...
    if nen == 4:
        ...
        N[0] = 0.25 * (1 - e) * (1 - n) * (-e - n - 1)
        N[1] = 0.25 * (1 + e) * (1 - n) * (e - n - 1)
        N[2] = 0.25 * (1 + e) * (1 + n) * (e + n - 1)
        N[3] = 0.25 * (1 - e) * (1 + n) * (-e + n - 1)
        N[4] = 0.5 * (1 - e * e) * (1 - n)
        N[5] = 0.5 * (1 + e) * (1 - n * n)
        N[6] = 0.5 * (1 - e * e) * (1 + n)
        N[7] = 0.5 * (1 - e) * (1 - n * n)
...
C4[0, 0, 0, 0] = C4[1, 1, 1, 1] = lam + 2 * mu
C4[0, 0, 1, 1] = C4[1, 1, 0, 0] = lam
C4[0, 1, 0, 1] = C4[1, 0, 0, 1] = C4[0, 1, 1, 0] = C4[1, 0, 1, 0] = mu
...
def von_mises_return(eps2, state):
    ...
    s_trial = (
        2.0 * mu * (eps - ep - (torch.trace(eps - ep) / 3.0) * I3)
        + K * torch.trace(eps) * I3
    )
...
def yield_curve_sigma12_closed(alpha, beta, n=361):
    ...
    for i, th in enumerate(thetas):
        v = sig_eff * (
            np.cos(th) / np.sqrt(lam1) * e1 + np.sin(th) / np.sqrt(lam2) * e2
        )
        xs[i] = v[0] + b11
        ys[i] = v[1] + b22
...
def clone_state_gp(state_gp):
    return [
        [
            {k: v.clone() if torch.is_tensor(v) else v for k, v in qp.items()}
            for qp in el
        ]
        for el in state_gp
    ]
...
state_gp = [
    [{"ep": torch.zeros(3, 3), "k": 0.0, "a": torch.zeros(3, 3)} for q in range(nqp)]
    for e in range(nel)
]
u = torch.zeros(nnp * ndf, 1)
free_dofs = torch.nonzero(
    1
    - torch.zeros(nnp * ndf, 1).index_fill_(0, (drlt[:, 0] * 2 + drlt[:, 1]).long(), 1)
)[:, 0]
...
while step <= n_steps:
    ...
    while True:
        ...
        print(
            f"  Substep: fac_conv={fac_conv:.4f} -> fac={fac:.4f} (scale={fac_scale:.3e})"
        )
        ...
        for it in range(newton_max):
            ...
            for el in range(nel):
                ...
                for q in range(nqp):
                    ...
                    sig, Ct, state_gp_current_substep[el][q] = von_mises_return(
                    ...
                    if (
                        track_el is None
                        and it == 0
                        and torch.norm(f_ext[free_dofs]).item() > 1.0
                    ):
                        ...
                        r_el = float(
                            torch.mean(torch.sqrt(xe[0, :] ** 2 + xe[1, :] ** 2))
                        )
                    ...
                    for A in range(nen):
                        ...
                        fe[A * 2 : A * 2 + 2, 0] += (
                            dv * (sig[:2, :2] @ G[A].unsqueeze(1)).squeeze()
                        )
            ...
            if rel < newton_tol:
                ...
                if track_el is not None:
                    if it == 0:
                        print(
                            f"Tracking: El {track_el} QP {track_q} (Max Stress approx {best_val/1e6:.1f} MPa)"
                        )
                ...
                if track_el is not None:
                    ...
                    G = torch.linalg.solve(Je.T, Gsh.T).T
                    eps = 0.5 * (ue @ G + (ue @ G).t())
                    sig_fin, _, _ = von_mises_return(
                        eps, state_gp_old[track_el][track_q]
                    )  # Use converged state input
                    ...
                    if abs(s11 - s22) > 1e-9:
                        ...
                        theta_p = (
                            0.0
                            if abs(s12) < 1e-9
                            else (math.pi / 4 if s12 > 0 else -math.pi / 4)
                        )
        ...
        if converged:
            ...
            if fac_scale < 1e-4 or cutbacks > 8:
                raise RuntimeError(
                    f"Step {step} failed to converge after {cutbacks} cutbacks."
                )
...
if interactive_hover:
    ...
    ann3.set_visible(False)

    def hover(event):
        for ax, ann, pc, key, unit, f in [
            (ax2, ann2, pc_u, "u_norm", "mm", fig_u),
            (ax3, ann3, pc_s, "svm", "MPa", fig_s),
        ]:
            if event.inaxes == ax:
...
plt.subplot(2, 2, 1)
plt.plot(disp_pl, np.array(load_hist) / 1000.0, "r", label="Actual")
...
if len(a_hist) > 0:
    ...
    sig_range = np.linspace(
        np.min(xs_cyc / 1e6) - 1000, np.max(xs_cyc / 1e6) + 1000, 100
    )

    # Line 1 & 2
    plt.plot(
        sig_range,
        sig_range + (shift / 1e6) + (limit_dist / 1e6),
        "r:",
        alpha=0.6,
        label="PE Limit",
    )
    ...
    # Bounds logic
    x_min_surf = np.min(xs_cyc / 1e6)
    x_max_surf = np.max(xs_cyc / 1e6)
    y_min_surf = np.min(ys_cyc / 1e6)
    y_max_surf = np.max(ys_cyc / 1e6)
    ...
    plt.figtext(
        0.5,
        0.01,
        f"Simulation Duration: {sim_duration:.2f} s",
        ha="center",
        fontsize=9,
...
plt.tight_layout()
try:
    fig_u.savefig(os.path.join(script_dir, "displacement_results.png"), dpi=300)
    fig_s.savefig(os.path.join(script_dir, "stress_results.png"), dpi=300)
